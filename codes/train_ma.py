        ...
        for _, train_data in enumerate(train_loader):
            ...
            if current_step % opt['train']['val_freq'] == 0:
                ...
                for val_data in val_loader:
                    ...
                    SR_img_path.append(save_img_path)

                score = eng.calc_Ma(SR_img_path)
                ...
                # PI
                logger.info('# Validation # PSNR: {:.4e}, NIQE: {:.4e}, Ma: {:.4e}, PI: {:.4e}, pirm_rmse: {:.4e}'.format(avg_psnr,NIQE,Ma,PI,avg_pirm_rmse))
                logger_val = logging.getLogger('val')  # validation logger
                logger_val.info('<epoch:{:3d}, iter:{:8,d}> psnr: {:.4e}, NIQE: {:.4e}, Ma: {:.4e}, PI: {:.4e}, pirm_rmse: {:.4e}'.format(
                    epoch, current_step, avg_psnr,NIQE,Ma,PI,avg_pirm_rmse))
                # tensorboard logger
                if opt['use_tb_logger'] and 'debug' not in opt['name']:
                    tb_logger.add_scalar('psnr', avg_psnr, current_step)
                    tb_logger.add_scalar('NIQE', NIQE, current_step)
                    tb_logger.add_scalar('Ma', Ma, current_step)
                    tb_logger.add_scalar('PI', PI, current_step)
