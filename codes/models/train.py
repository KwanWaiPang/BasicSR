    ...
    for epoch in range(start_epoch, total_epochs):
        for _, train_data in enumerate(train_loader):
            ...
            if current_step % opt['train']['val_freq'] == 0:
                ...
                # log
                logger.info('# Validation # PSNR: {:.4e}, NIQE: {:.4e}, pirm-rmse: {:.4e}'.format(avg_psnr,scores,avg_pirm_rmse))
                logger_val = logging.getLogger('val')  # validation logger
                logger_val.info('<epoch:{:3d}, iter:{:8,d}> psnr: {:.4e}, NIQE: {:.4e}, pirm-rmse: {:.4e}'.format(
                    epoch, current_step, avg_psnr, scores, avg_pirm_rmse))
                # tensorboard logger
                if opt['use_tb_logger'] and 'debug' not in opt['name']:
                    tb_logger.add_scalar('psnr', avg_psnr, current_step)
                    tb_logger.add_scalar('NIQE', scores, current_step)
                    tb_logger.add_scalar('pirm-rmse', avg_pirm_rmse, current_step)
