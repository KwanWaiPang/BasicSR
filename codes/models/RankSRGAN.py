...
class SRGANModel(BaseModel):
    def __init__(self, opt):
        ...
        if self.is_train:
            ...
            self.netG.train()
            self.netD.train()
            self.netR = networks.define_R(opt).to(self.device)  # E
    ...
    def optimize_parameters(self, step):
        ...
        if step % self.D_update_ratio == 0 and step > self.D_init_iters:
            ...
            l_g_R = self.netR(self.fake_H)
            l_g_R = torch.sigmoid(l_g_R-self.rbias)
            l_g_R = torch.sum(l_g_R)
            l_g_R = self.l_R_w*l_g_R   # 0.05   hr0.2 ESRGAN 0.1，0.03
            l_g_total += l_g_R
        ...
        if step % self.D_update_ratio == 0 and step > self.D_init_iters:
            ...
            self.log_dict['l_g_R'] = l_g_R.item()
