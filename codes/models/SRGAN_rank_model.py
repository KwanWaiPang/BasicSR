...
class SRGANModel(BaseModel):
    ...
    def __init__(self, opt):
        ...
        self.netE = networks.define_R(opt).to(self.device)  # E
        ...
        if self.is_train:
            self.rbias = train_opt['R_bias']
            self.l_R_w = train_opt['R_weight']
            # G pixel loss
    ...
    def optimize_parameters(self, step):
        ...
        if step % self.D_update_ratio == 0 and step > self.D_init_iters:
            ...
            l_g_eloss = self.netE(self.fake_H)
            # l_testval = torch.sum(l_g_eloss)
            # print('---------go----------')
            # print(l_g_eloss)

            # print(l_testval)
            l_g_eloss = torch.sigmoid(l_g_eloss-self.rbias)
            # softplusAF = torch.nn.Softplus()
            # l_g_eloss = softplusAF(l_g_eloss)
            # print(l_g_eloss)
            # l_g_eloss_bias = l_g_eloss+0.2
            # print(l_g_eloss_bias)
            # l_g_eloss_test = torch.sum(l_g_eloss_bias)
            # print(l_g_eloss_test)
            # for val in l_g_eloss:
                # val_exp = math.exp(val)
                # l_g_eloss_exp.append(val_exp)
                # print(val_exp)
            # l_g_eloss = math.exp(l_g_eloss)

            # print('----after Softplus------')
            # print(l_g_eloss_exp)
            # l_g_eloss = l_g_eloss+0.5
            l_g_eloss = torch.sum(l_g_eloss)
            # print('----after sum------')
            # print(l_g_eloss)
            l_g_eloss = self.l_R_w*l_g_eloss   # 0.05   hr0.2 ESRGAN 0.1，0.03
            # print('----after weight------')
            # print(l_g_eloss)
            l_g_total += l_g_eloss
        ...
        self.log_dict['l_g_eloss'] = l_g_eloss.item()
        # D
    ...
    def load(self):
        ...
        if load_path_E is not None:
            print('loading model for E [%s] ...' % load_path_E)
